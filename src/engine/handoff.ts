...
export function normalizeHandoffReport(
  value: unknown,
  fallback: { handoffId: string; generatedAt?: string }
...
function toMarkdown(report: HandoffReport): string {
  const lines: string[] = [];
  const pickupSummary =
    report.handoff.lifecycle.pickup_by && report.handoff.lifecycle.pickup_at
      ? `${report.handoff.lifecycle.pickup_by} at ${report.handoff.lifecycle.pickup_at}`
      : "unclaimed";
  ...
  lines.push(`Handoff ID: ${sanitizeMd(report.handoff.id)}`);
  lines.push(`Target: ${sanitizeMd(report.handoff.to ?? "unassigned")}`);
  lines.push(`Lifecycle: ${sanitizeMd(report.handoff.lifecycle.status)}`);
  lines.push(`Pickup: ${sanitizeMd(pickupSummary)}`);
  lines.push("");
...
export function generateHandoff(options: { to?: string } = {}): {
  ...
  const report: HandoffReport = {
    ...
    handoff: {
      id: baseName,
      to: normalizeHandoffTarget(options.to),
      lifecycle: {
        status: "open",
