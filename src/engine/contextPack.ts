...
export function parseDecisionsMarkdown(content: string, file = ".tack/decisions.md"): DecisionEntry[] {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    // The decision/reasoning separator is one or more hyphens with whitespace on BOTH
    // sides (`-` or a hand-typed `--`), or an em dash. A bare hyphen must not qualify:
    // the lazy `(.+?)` would otherwise stop at the first hyphenated word
    // ("session-first") and hand its tail to the reasoning.
